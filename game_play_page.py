...
class GamePage(BasePage):
    ...
    def apply_rule_11_and_click(self, grid_2d, color_map, queens_grid, size_x, size_y):
        """
        Rule 11 (Fallback):
        If a color group has no queen and has available '1' spots (from JSON),
        place a queen on one of those cells.
        Indexing is column-major to match other rules (like Rule 10).
        """
        new_grid = deepcopy(grid_2d)
        loc_queen = None
        candidates = []

        # Step 1: Collect color groups without a queen
        for color, cells in color_map.items():
            empty_cells = []
            has_queen = False
        ...
        # Step 2: Pick color with minimum empty cells (like fallback)
        min_empty = float('inf')
        selected_group = None
        for color, empty_cells, all_cells in candidates:
            if len(empty_cells) < min_empty:
        ...
        # Step 3: Fix invalid X marks — match Rule 10's column-major index
        for r in range(size_y):
            for c in range(size_x):
                vertical_idx_invalid = c * size_y + r  # ✅ column-major like Rule 10
                if new_grid[r][c] == 'x' and str(queens_grid[vertical_idx_invalid]) == '1':
                    partial_name = f"Grid-{vertical_idx_invalid}-"
                    element = self.altdriver.find_object_which_contains(By.NAME, partial_name)
                    self.sleep()
        ...
        # Step 4: Try to place queen where JSON says '1' and grid is empty
        for r, c in all_cells:
            vertical_idx = c * size_y + r  # ✅ same as Rule 10
            if str(queens_grid[vertical_idx]) == '1' and new_grid[r][c] == 'e':
                new_grid[r][c] = 'q'
                loc_queen = (r, c)
                print(f"👑 Rule 11 placed queen at ({r},{c}), index={vertical_idx}")
                ...
                try:
                    ...
                    self.sleep()
                    element.tap()
                    element.tap()  # double tap just in case
                except Exception as e:
                    print(f"❌ Tap failed in Rule 11: {e}")
