...
class TestIOSAnalytics(TestBase):
    install_id = None
    start_page = None
    ...
    def setUpClass(cls):
        ...
        cls.options.set_capability("appium:noReset", True)
        ...
        cls.altdriver = None
        print(">>> Setting up TestIOSAnalytics class")

    def setUp(self):

        print(f">>> Restarting iOS app before {self._testMethodName}")

        # ✅ Restart iOS app via Appium
        ...
        TestIOSAnalytics.start_page = StartPage(self.altdriver, self.appium_driver)
    ...
    def test_Analytics_sdk_init(self):
        ...
        TestIOSAnalytics.install_id = install_id
        ...
        # ✅ THIS is what actually fails the test if no records were found
        assert result is not None, "❌ Result is None"

    @pytest.mark.order(2)
    def test_Analytics_level_started(self):
        install_id = TestIOSAnalytics.install_id
    ...
    def test_Analytics_idfa_acceptance(self):
        install_id = TestIOSAnalytics.install_id
        assert install_id is not None, "install_id was not set by test_Analytics_sdk_init!"
        ...
        for attempt in range(1, max_retries + 1):
            ...
            result = client.getSdkInitEventsByInstallIdAndDate(install_id, "idfa_acceptance")
            print(f"Result from Databricks: {result}")
